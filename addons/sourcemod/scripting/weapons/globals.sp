...
const int MAX_UNLOCKED_ITEMS = 512;

enum CosmeticsMode
{
	CosmeticsMode_Drops = 0,
	CosmeticsMode_Inventory,
	CosmeticsMode_Hybrid
};
...
ConVar g_Cvar_EnablePublicWS;
bool g_bEnablePublicWS;
...
ConVar g_Cvar_DropChance;
float g_fDropChance;

ConVar g_Cvar_CosmeticsMode;
CosmeticsMode g_iCosmeticsMode = CosmeticsMode_Drops;

ConVar g_Cvar_DropShowRollStats;
bool g_bDropShowRollStats;

ConVar g_Cvar_DropDebug;
bool g_bDropDebug;

ConVar g_Cvar_DropStatTrakChance;
float g_fDropStatTrakChance;

ConVar g_Cvar_DropSeedMin;
int g_iDropSeedMin;

ConVar g_Cvar_DropSeedMax;
int g_iDropSeedMax;

ConVar g_Cvar_DropWearDefaultMin;
float g_fDropWearDefaultMin;

ConVar g_Cvar_DropWearDefaultMax;
float g_fDropWearDefaultMax;

ConVar g_Cvar_DropWeightKnife;
float g_fDropWeightKnife;

ConVar g_Cvar_DropWeightAK47;
float g_fDropWeightAK47;

ConVar g_Cvar_DropWeightM4A1;
float g_fDropWeightM4A1;

ConVar g_Cvar_DropWeightM4A1S;
float g_fDropWeightM4A1S;

ConVar g_Cvar_DropWeightPistol;
float g_fDropWeightPistol;

ConVar g_Cvar_DropWeightOther;
float g_fDropWeightOther;

ConVar g_Cvar_InventoryImportSourceTable;
char g_InventoryImportSourceTable[64];

ConVar g_Cvar_InventoryImportSyncInterval;
float g_fInventoryImportSyncInterval;
Handle g_hInventoryImportTimer = INVALID_HANDLE;

ConVar g_Cvar_InventoryBackendUrl;
char g_InventoryBackendUrl[256];

ConVar g_Cvar_InventoryBackendApiKey;
char g_InventoryBackendApiKey[128];

ConVar g_Cvar_InventorySteamId64Override;
char g_InventorySteamId64Override[32];

int g_iWeaponIndexAK47 = -1;
int g_iWeaponIndexM4A1 = -1;
int g_iWeaponIndexM4A1S = -1;
...
bool g_bBypassBuyCommandRemap[MAXPLAYERS+1] = { false, ... };
int g_iSeedRandom[MAXPLAYERS+1][sizeof(g_WeaponClasses)];

int g_iKnife[MAXPLAYERS+1] = { 0, ... };

int g_iUnlockedItemCount[MAXPLAYERS+1] = { 0, ... };
int g_iUnlockedItemId[MAXPLAYERS+1][MAX_UNLOCKED_ITEMS];
int g_iUnlockedItemWeaponIndex[MAXPLAYERS+1][MAX_UNLOCKED_ITEMS];
int g_iUnlockedItemSkinId[MAXPLAYERS+1][MAX_UNLOCKED_ITEMS];
float g_fUnlockedItemWear[MAXPLAYERS+1][MAX_UNLOCKED_ITEMS];
int g_iUnlockedItemSeed[MAXPLAYERS+1][MAX_UNLOCKED_ITEMS];
int g_iUnlockedItemStatTrakEnabled[MAXPLAYERS+1][MAX_UNLOCKED_ITEMS];
int g_iUnlockedItemStatTrakCount[MAXPLAYERS+1][MAX_UNLOCKED_ITEMS];
int g_iEquippedItemId[MAXPLAYERS+1][sizeof(g_WeaponClasses)];

int g_iCTDefaultPistolVariant[MAXPLAYERS+1] = { 6, ... };
int g_iCTM4Variant[MAXPLAYERS+1] = { 2, ... };
int g_iCTAutoPistolVariant[MAXPLAYERS+1] = { 11, ... };
int g_iTAutoPistolVariant[MAXPLAYERS+1] = { 12, ... };
int g_iHeavyPistolVariant[MAXPLAYERS+1] = { 4, ... };
int g_iMidSMGVariant[MAXPLAYERS+1] = { 22, ... };
...
StringMap g_smWeaponSkinIndex;
StringMap g_smLanguageIndex;
StringMap g_smSkinWearMin;
StringMap g_smSkinWearMax;
StringMap g_smSkinDisplayName;
StringMap g_smSkinDisplayNameFallback;
StringMap g_smSkinIndexByNameAndWeapon;
